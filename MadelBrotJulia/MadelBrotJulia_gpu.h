...
double CalculatePoint_gpu(double x, double y, double xc, double yc, int iter){
...
unsigned char* CalculateMadel_gpu(int iter, double* info,  unsigned char* ThePic , bool ToJulia ){
    ...
    for(int j = 0;  j < rheight ; j++){
        ...
        for(int i = 0;  i < rwidth ; i++){
            ...
            if(ToJulia){        //  Skaiciuojame Julia
                Rez = CalculatePoint_gpu( xc , yc, Jx , Jy, iter) ;
             }else{         // SKaiciuojame Mandel
                Rez = CalculatePoint_gpu(0 , 0, xc, yc, iter) ;
             }
